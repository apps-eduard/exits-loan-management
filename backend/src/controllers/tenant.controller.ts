...
export class TenantController {
   ...
   */
  getTenantsForTesting = async (req: Request, res: Response): Promise<void> => {
    try {
      const tenants = await this.tenantService.getTenantsForTesting();
      
      console.log(`🏢 Found ${tenants.length} tenants for testing:`, 
        tenants.map(t => ({ company: t.companyName, slug: t.slug, userCount: t.users.length }))
      );
      ...
      console.error('Error fetching tenants for testing:', error);
      res.status(500).json({ error: 'Failed to fetch tenants for testing' });
    }
  };
}
